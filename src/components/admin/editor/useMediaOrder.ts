...
/**
 * Debounced order saver.
 * Expected API route: PATCH /api/admin/posts/{slug}/media/reorder
 * Body: { order: string[] }
 *
 * If you prefer postId-based endpoints, adjust save() accordingly.
 */
  ...
  const save = React.useCallback(
    async (orderedIds: string[]) => {
      ...
      const res = await fetch(
        `/api/admin/posts/${encodeURIComponent(slug)}/media/reorder`,
        {
          method: "PATCH",
